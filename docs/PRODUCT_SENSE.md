...
- Rust-only local runtime (`pty-rust`) with an embedded local terminal in `discode tui`
- Rust native attach client retained as a deterministic fallback local terminal UI
